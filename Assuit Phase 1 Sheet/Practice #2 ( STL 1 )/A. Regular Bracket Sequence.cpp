...
//Solution Here
int main(){
    int opened_brackets = 0, valid_brackets = 0;
    string s;
    cin >> s;
    for(int i = 0 ; i < s.size() ; i++){
        if(opened_brackets == 0 && s[i] == ')') continue;
        else if(s[i] == ')' && opened_brackets != 0){
            valid_brackets++;
            opened_brackets--;
        }
        else if(s[i] == '(') opened_brackets++;
    }
    cout << valid_brackets*2 << endl;
return 0;}
